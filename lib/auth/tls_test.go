...
func TestGenerateCerts(t *testing.T) {
	...
	t.Run("ImpersonateDeny", func(t *testing.T) {
		// User can't generate certificates for another user by defrault
	...
	t.Run("ImpersonateAllow", func(t *testing.T) {
		...
		role, err := services.NewRole("impersonate", types.RoleSpecV3{
			Allow: types.RoleConditions{
				Logins: []string{user2.GetName()},
				Impersonate: &types.ImpersonateConditions{
					Users: []string{user2.GetName()},
					Roles: []string{userRole2.GetName()},
				},
			},
		})
		require.NoError(t, err)
